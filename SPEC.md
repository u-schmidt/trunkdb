...
large-document workload (§5.2) the second; see §42 for the roadmap.
...
A pattern language (regex, `LIKE` wildcards) is still open (§42.2). A
...
(§42). It's also a backup that can be read and `diff`ed, and a way to
...
- Writers wait for the whole export. For a large database that's
  seconds; a snapshot that doesn't block writers needs MVCC (§42.2).
...
- Still one field per index: compound indexes are still open (§42.2);
...
- An AND uses one part's bounds, never the intersection of several
  indexes' ranges.
- Still no pattern language, no conditions on array elements (§42.2).
...
- **Damage in the header or the catalog stops `open`**, so `check` can't
  run on such a file. Repairing is open (§42.2).
...
| next | Compaction, and index leaves that fill when keys come in order | §41 |
